...
import { SWEETNESS_OPTIONS } from '../constants'

const router = useRouter()
const { customSweetness, addSweetness, deleteSweetness } = useCustomOptions()
...
async function onAdd() {
  if (!newLabel.value.trim()) { showToast('请输入甜度名称'); return }
  adding.value = true
  try {
    await addSweetness(newLabel.value.trim())
    newLabel.value = ''
    showAdd.value = false
    showToast('添加成功')
...
<template>
  <div class="manage-page">
    <div class="manage-header">
      ...
      <h2 class="manage-title">我的甜度</h2>
      <van-icon name="plus" class="add-btn" @click="showAdd = true" />
    </div>

    <div class="manage-section">
      <div class="manage-section-title">默认甜度</div>
      <div class="option-list">
        <div v-for="s in SWEETNESS_OPTIONS" :key="s" class="option-row">
    ...
    <div class="manage-section" v-if="customSweetness.length">
      <div class="manage-section-title">自定义甜度</div>
      <div class="option-list">
        <div v-for="s in customSweetness" :key="s.id" class="option-row">
          <span class="option-label">{{ s.label }}</span>
          <van-icon name="delete-o" class="delete-icon" @click="onDelete(s.id, s.label)" />
        </div>
      </div>
    </div>

    <van-popup v-model:show="showAdd" position="bottom" round class="add-popup">
      <div class="add-popup-header">添加甜度</div>
      <van-cell-group inset>
        <van-field v-model="newLabel" label="名称" placeholder="如：八分糖" @keyup.enter="onAdd" />
      </van-cell-group>
